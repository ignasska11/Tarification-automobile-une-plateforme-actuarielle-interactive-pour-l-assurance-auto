...
.small-box {
  border-radius: var(--radius) !important;
  box-shadow: 0 1px 4px rgba(83,74,183,0.08) !important;
}
...
.bg-violet  { background: linear-gradient(135deg, var(--violet-dark), var(--violet)) !important; }
.bg-teal-c  { background: linear-gradient(135deg, #0F6E56, var(--teal)) !important; }
.bg-coral-c { background: linear-gradient(135deg, #993C1D, var(--coral)) !important; }
.bg-amber-c { background: linear-gradient(135deg, #854F0B, var(--amber)) !important; }
...
table.dataTable thead th {
  background: var(--violet-lite) !important;
  color: var(--violet-dark) !important;
...
VBOX <- function(value, subtitle, icon_fa, color = "primary") {
  col_map <- c(primary = "primary", violet = "primary",
               teal = "success", coral = "danger",
               amber = "warning", gray = "secondary")
...
# ==============================================================================
# 4. THEME PLOTLY  -  fond blanc, police Inter
# ==============================================================================
...
PT <- function(p, ...) {
  p %>% plotly::layout(
    ...
    xaxis = list(
      gridcolor    = "rgba(83,74,183,0.08)",
      zerolinecolor = "rgba(83,74,183,0.15)",
      tickfont     = list(color = "#6B6A7A"),
      linecolor    = "rgba(83,74,183,0.1)"
    ),
    yaxis = list(
      gridcolor    = "rgba(83,74,183,0.08)",
      zerolinecolor = "rgba(83,74,183,0.15)",
    ...
    legend = list(
      font        = list(color = "#6B6A7A", size = 11),
    ...
    plotly::config(
      ...
      toImageButtonOptions = list(
        format = "png", filename = "autoActuariat_graph",
        height = 500, width = 900, scale = 2)
    )
}

# ==============================================================================
# 5. PAGE : ACCUEIL CLIENT
# ==============================================================================
...
page_accueil_client <- bs4TabItem("accueil_client",
                                    ...
                                    column(12,
                                           ...
                                           div(style = "position: relative; z-index: 1;",
                                               tags$i(class = "fas fa-car",
                                                      style = "font-size: 3rem; margin-bottom: 16px; display: block; opacity: 0.9;"),
                                               h2(style = "font-size: 2.2rem; font-weight: 600; margin: 0 0 8px; color: #fff;",
                                                  "Bienvenue dans votre espace"),
                                               p(style = "font-size: 1rem; opacity: 0.8; margin: 0 0 28px; max-width: 500px; margin-left: auto; margin-right: auto;",
                                                 "Simulez votre prime d'assurance auto en quelques secondes et visualisez votre positionnement dans notre portefeuille."),
                                               div(style = "display: flex; gap: 12px; justify-content: center; flex-wrap: wrap;",
                                                   actionButton("go_simulateur", "Simuler ma prime",
                                                                class = "btn btn-light",
                                                                style = "font-weight: 500; padding: 10px 24px; border-radius: 8px;",
                                                                icon = icon("calculator")),
                                                   actionButton("go_position", "Ma position",
                                                                style = paste0(
                                                                  "background: rgba(255,255,255,0.15); color: white;",
                                                                  "border: 1px solid rgba(255,255,255,0.3);",
                                                                  "font-weight: 500; padding: 10px 24px; border-radius: 8px;"),
                                                                icon = icon("chart-pie"))
                                               )
                                           )
                                           )
                                  ...
                                  fluidRow(
                                    column(4,
                                           div(style = paste0(
                                             "background: white; border: 1px solid rgba(83,74,183,0.12);",
                                           ...
                                           tags$i(class = "fas fa-sliders",
                                                  style = "font-size: 2rem; color: #534AB7; margin-bottom: 12px; display: block;"),
                                           h4(style = "font-weight: 600; margin: 0 0 6px; font-size: 1rem;", "Mon devis"),
                                           p(style = "font-size: 0.82rem; color: #6B6A7A; margin: 0;",
                                             "Calculez votre prime personnalisée à partir de votre profil.")
                                           )
                                    ),
                                    column(4,
                                           div(style = paste0(
                                           ...
                                           tags$i(class = "fas fa-chart-pie",
                                                  style = "font-size: 2rem; color: #1D9E75; margin-bottom: 12px; display: block;"),
                                           h4(style = "font-weight: 600; margin: 0 0 6px; font-size: 1rem;", "Ma position"),
                                           p(style = "font-size: 0.82rem; color: #6B6A7A; margin: 0;",
                                             "Comparez votre profil au reste du portefeuille.")
                                           )
                                    ),
                                    column(4,
                                           div(style = paste0(
                                           ...
                                           tags$i(class = "fas fa-shield-halved",
                                                  style = "font-size: 2rem; color: #BA7517; margin-bottom: 12px; display: block;"),
                              ...
                              fluidRow(
                                column(4,
                                       box(width = 12, collapsible = FALSE,
                                           title = span(
                                             tags$i(class = "fas fa-sliders", style = "color:#534AB7;margin-right:6px;"),
                                           ...
                                           div(class = "section-badge", style = "margin-bottom:12px;",
                                               tags$i(class="fas fa-user"), "Conducteur"),
                                           ...
                                           # Vehicule
                                           div(class = "section-badge", style = "margin-bottom:12px;",
                                               tags$i(class="fas fa-car"), "Vehicule"),
                                           sliderInput("s_vehage", "Age du vehicule (ans)", 0, 20, 5, 1),
                                           sliderInput("s_power",  "Puissance fiscale (CV)", 4, 15, 7, 1),
                                           selectInput("s_gas", "Carburant",
                                                       choices = c("Diesel" = "Diesel", "Essence" = "Regular"),
                                                       selected = "Regular"),
                                           tags$hr(style = "border-color: rgba(83,74,183,0.1); margin: 10px 0;"),
                                           # Localisation
                                           div(class = "section-badge", style = "margin-bottom:12px;",
                                               tags$i(class="fas fa-map-pin"), "Localisation"),
                                           ...
                                           br(),
                                           actionButton("btn_calc", "Calculer ma prime",
                                                        class = "btn-primary w-100", icon = icon("bolt")),
                                           br(), br(),
                                           downloadButton("btn_dl", "Telecharger mon devis",
                                                          class = "btn-success w-100")
                                       )
                                ),
                                column(8,
                                       # Resultat principal
                                       uiOutput("sim_result_card"),
                                       br(),
                                       fluidRow(
                                         DBOX(plotlyOutput("sim_position", height = "210px"),
                                              title = "Votre position vs portefeuille",
                                              icon_fa = "users", status = "primary", width = 6,
                                              collapsible = FALSE, maximizable = FALSE),
                                         DBOX(plotlyOutput("sim_decomp", height = "210px"),
                                              title = "Decomposition de la prime",
                                              icon_fa = "chart-pie", status = "info", width = 6,
                                              collapsible = FALSE, maximizable = FALSE)
                                       ),
                                       fluidRow(
                                         column(12, uiOutput("sim_conseil"))
                                       )
                                )
                              )
)
...
# ==============================================================================
# 7. PAGE : POSITION CLIENT
# ==============================================================================

page_client_pos <- bs4TabItem("client_pos",
                              fluidRow(
                                DBOX(plotlyOutput("client_bm_pos",  height = "270px"),
                                     title = "Votre BM vs portefeuille", icon_fa = "star",
                                     status = "primary", width = 6),
                                DBOX(plotlyOutput("client_age_pos", height = "270px"),
                                     title = "Sinistralite selon votre age", icon_fa = "user",
                                     status = "info", width = 6)
                              ),
                              fluidRow(
                                DBOX(plotlyOutput("client_zone_pos", height = "270px"),
                                     title = "Prime par zone geographique", icon_fa = "map",
                                     status = "warning", width = 6),
                                DBOX(uiOutput("client_conseil_full"),
                                     title = "Vos conseils personnalises", icon_fa = "lightbulb",
                                     status = "success", width = 6, collapsible = FALSE)
                              )
)

# ==============================================================================
# 8. PAGE : ACCUEIL ASSUREUR
# ==============================================================================
                                    ...
                                    fluidRow(
                                      column(12,
                                             ...
                                             p(style = "font-size: 1rem; opacity: 0.8; margin: 0 0 24px; max-width: 560px; margin-left: auto; margin-right: auto;",
                                               "Acces complet aux modeles GLM & XGBoost, aux outils XAI, calibration, cartographie regionale et rapports exportables."),
...
# ==============================================================================
# 9. PAGE : DASHBOARD ASSUREUR
# ==============================================================================

page_dashboard <- bs4TabItem("dashboard",
                             fluidRow(
                               valueBoxOutput("vb_polices",  width = 3),
                               valueBoxOutput("vb_freq_obs", width = 3),
                               valueBoxOutput("vb_sev_moy",  width = 3),
                               valueBoxOutput("vb_prime_med",width = 3)
                             ),
                             fluidRow(
                               valueBoxOutput("vb_gini_xgb", width = 3),
                               valueBoxOutput("vb_auc_best", width = 3),
                               valueBoxOutput("vb_disp",     width = 3),
                               valueBoxOutput("vb_theta",    width = 3)
                             ),
                             fluidRow(
                               DBOX(plotlyOutput("dash_freq_age",  height = "280px"),
                                    title = "Frequence exposee par classe d'age",
                                    icon_fa = "user", status = "primary", width = 8),
                               DBOX(plotlyOutput("dash_donut",     height = "280px"),
                                    title = "Sinistralite globale",
                                    icon_fa = "chart-pie", status = "warning", width = 4)
                             ),
                             fluidRow(
                               DBOX(plotlyOutput("dash_bm",      height = "250px"),
                                    title = "Frequence vs Bonus-Malus",
                                    icon_fa = "star", status = "info", width = 4),
                               DBOX(plotlyOutput("dash_zone",    height = "250px"),
                                    title = "Frequence par zone geographique",
                                    icon_fa = "map", status = "warning", width = 4),
                               DBOX(plotlyOutput("dash_lorenz",  height = "250px"),
                                    title = "Courbe de Lorenz (3 modeles)",
                                    icon_fa = "chart-line", status = "success", width = 4)
                             )
)
...
page_modeles <- bs4TabItem("modeles",
                           fluidRow(
                             DBOX(DTOutput("tbl_comp_freq"),
                                  title = "Comparaison modeles de frequence",
                                  icon_fa = "trophy", status = "primary", width = 12)
                           ),
                           fluidRow(
                             DBOX(plotlyOutput("lorenz_3",    height = "280px"),
                                  title = "Courbes de Lorenz (3 modeles)",
                                  icon_fa = "chart-line", status = "primary", width = 6),
                             DBOX(plotlyOutput("roc_plot",    height = "280px"),
                                  title = "Courbes ROC",
                                  icon_fa = "chart-area", status = "info", width = 6)
                           ),
                           fluidRow(
                             DBOX(DTOutput("tbl_comp_sev"),
                                  title = "Comparaison modeles de severite",
                                  icon_fa = "coins", status = "info", width = 12)
                           ),
                           fluidRow(
                             DBOX(plotlyOutput("scatter_obs_sev", height = "260px"),
                                  title = "Observe vs Predit (Gamma)",
                                  icon_fa = "chart-scatter", status = "success", width = 6),
                             DBOX(plotlyOutput("prime_bm_line",   height = "260px"),
                                  title = "Prime pure vs Bonus-Malus (PDP)",
                                  icon_fa = "chart-line", status = "warning", width = 6)
                           )
)
...
page_xai <- bs4TabItem("xai",
                       fluidRow(
                         DBOX(plotlyOutput("shap_bar",    height = "320px"),
                              title = "SHAP — Importance globale (frequence)",
                              icon_fa = "eye", status = "primary", width = 6),
                         DBOX(plotlyOutput("coef_plot",   height = "320px"),
                              title = "Coefficients GLM BN — Effets multiplicatifs",
                              icon_fa = "list-check", status = "info", width = 6)
                       ),
                       fluidRow(
                         DBOX(plotlyOutput("pdp_age",     height = "260px"),
                              title = "PDP — Age du conducteur",
                              icon_fa = "user", status = "primary", width = 6),
                         DBOX(plotlyOutput("pdp_bm",      height = "260px"),
                              title = "PDP — Bonus-Malus",
                              icon_fa = "star", status = "warning", width = 6)
                       ),
                       fluidRow(
                         DBOX(plotlyOutput("ice_age",     height = "280px"),
                              title = "ICE — Age (50 polices, bandes P10-P90)",
                              icon_fa = "chart-line", status = "info", width = 6),
                         DBOX(plotlyOutput("ice_bm",      height = "280px"),
                              title = "ICE — Bonus-Malus (50 polices)",
                              icon_fa = "star", status = "primary", width = 6)
                       )
)
...
page_calibration <- bs4TabItem("calibration",
                               fluidRow(
                                 DBOX(DTOutput("tbl_calib"),
                                      title = "Calibration par decile — Ratio Observe/Predit",
                                      icon_fa = "bullseye", status = "success", width = 7),
                                 DBOX(plotlyOutput("plot_calib_bar", height = "300px"),
                                      title = "Ratio O/P par decile",
                                      icon_fa = "chart-bar", status = "success", width = 5)
                               ),
                               fluidRow(
                                 DBOX(plotlyOutput("lorenz_all",    height = "270px"),
                                      title = "Courbe de Lorenz — Comparaison",
                                      icon_fa = "chart-line", status = "primary", width = 6),
                                 DBOX(plotlyOutput("double_lift",   height = "270px"),
                                      title = "Double Lift — GLM BN vs XGBoost",
                                      icon_fa = "arrows-left-right", status = "info", width = 6)
                               )
)

# ==============================================================================
# 13. PAGE : CARTOGRAPHIE
# ==============================================================================

page_carto <- bs4TabItem("carto",
                         fluidRow(
                           valueBoxOutput("vb_carto_max",   width = 3),
                           valueBoxOutput("vb_carto_min",   width = 3),
                           valueBoxOutput("vb_carto_idf",   width = 3),
                           valueBoxOutput("vb_carto_corse", width = 3)
                         ),
                         fluidRow(
                           column(8,
                                  DBOX(leafletOutput("map_regions", height = "440px"),
                                       title = "Carte interactive — Frequence par region",
                                       icon_fa = "map-location-dot", status = "primary", width = 12)
                           ),
                           column(4,
                                  DBOX(plotlyOutput("carto_bar", height = "220px"),
                                       title = "Frequence par region",
                                       icon_fa = "chart-bar", status = "primary", width = 12),
                                  DBOX(uiOutput("carto_risk_table"),
                                       title = "Classes de risque",
                                       icon_fa = "table", status = "info", width = 12)
                           )
                         )
)

# ==============================================================================
# 14. PAGE : PROFILS TYPE
# ==============================================================================

page_profils <- bs4TabItem("profils",
                           fluidRow(
                             DBOX(DTOutput("tbl_profils"),
                                  title = "Tarification par profil type",
                                  icon_fa = "users", status = "primary", width = 12)
                           ),
                           fluidRow(
                             DBOX(plotlyOutput("profils_bar",   height = "280px"),
                                  title = "Prime pure par profil",
                                  icon_fa = "euro-sign", status = "primary", width = 6),
                             DBOX(plotlyOutput("profils_scatter", height = "280px"),
                                  title = "BonusMalus vs Prime pure",
                                  icon_fa = "chart-scatter", status = "info", width = 6)
                           )
)
                           ...
                           fluidRow(
                             column(12,
                                    div(style = paste0(
                                      ...
                                      "border-radius: 16px; padding: 48px; text-align: center; color: white;",
                                      "margin-bottom: 20px;"
                                    ),
                                    tags$i(class = "fas fa-file-code",
                                           style = "font-size: 3.5rem; margin-bottom: 16px; display: block; opacity: 0.9;"),
                                    h2(style = "font-size: 2rem; font-weight: 600; margin: 0 0 10px; color: #fff;",
                                       "Rapport Actuariel Complet"),
                                    p(style = "font-size: 0.9rem; opacity: 0.8; max-width: 520px; margin: 0 auto 24px;",
                                      "Telechargez le rapport HTML complet : resultats, modeles, metriques, profils et calibration."),
                                    downloadButton("dl_rapport", "Telecharger le rapport",
                                                   icon = icon("download"),
                                                   style = paste0(
